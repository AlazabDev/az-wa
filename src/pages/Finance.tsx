...
export default function Finance() {
  ...
  // Import an entire Supabase Storage folder (300+ files) page by page.
  const importMutation = useMutation({
    mutationFn: async () => {
      ...
      for (let page = 0; page < 60; page++) {
        ...
        setImportProgress({ done, created });
    ...
    onSuccess: (created) => {
      toast.success(`تم استيراد ${created} ملف من المخزن إلى Milano`);
  ...
  const workerMutation = useMutation({
    mutationFn: async ({ batchId, all }: { batchId: string; all?: boolean }) => {
      if (!currentTenantId) throw new Error("لا يوجد Tenant محدد");
      cancelRef.current = false;
      let processed = 0;
      let remaining = 0;
      const maxRounds = all ? 200 : 1;
      for (let i = 0; i < maxRounds; i++) {
        if (cancelRef.current) break;
        const { data, error } = await supabase.functions.invoke("finance-worker", {
          body: { tenant_id: currentTenantId, batch_id: batchId, limit: 5 },
        });
        if (error) throw error;
        ...
        qc.invalidateQueries({ queryKey: ["finance-documents"] });
        if (!all || remaining === 0 || (res.processed ?? 0) === 0) break;
      }
      return { processed, remaining };
    },
    onMutate: (v) => setAutoRun(Boolean(v.all)),
    onSettled: () => setAutoRun(false),
    onSuccess: (data) => {
      toast.success(`تمت معالجة ${data.processed} مستند — المتبقي ${data.remaining}`);
      invalidate();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const busy = importMutation.isPending || workerMutation.isPending;

...
function readButtons(meta: unknown): string[] {
  ...
  const buttonComponent = components.find(
    (c) => (c as { type?: string })?.type?.toUpperCase() === "BUTTONS",
  ) as { buttons?: { text?: string }[] } | undefined;
...
function readHeaderType(meta: unknown): string | undefined {
  ...
  const header = components.find(
    (c) => (c as { type?: string })?.type?.toUpperCase() === "HEADER",
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="space-y-4">
                <div>
                  ...
                  <Input placeholder="مثال: إشعار_صيانة" className="mt-1" />
                </div>
                <div>
                  <Label>حساب WABA</Label>
                  <Select>
                ...
                <div>
                  ...
                  <Textarea
                    placeholder="اكتب نص القالب... استخدم {{1}} للمتغيرات"
                    className="mt-1 min-h-[100px]"
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                    استخدم {"{{1}}"}, {"{{2}}"} أو {"{{param_name}}"} لإضافة متغيرات
                  </p>
                </div>
              </div>
              <div>
                ...
                <div className="rounded-2xl border bg-muted/30 p-4 min-h-[300px]">
                  <div className="bg-card rounded-xl p-4 shadow-card max-w-[250px] mr-auto">
                    <p className="text-sm leading-relaxed text-foreground">
                      مرحباً {"{{1}}"}، تم تحديد موعد الصيانة يوم {"{{2}}"}.
                    </p>
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-xs text-muted-foreground">
          {filtered.length} قالب من أصل {templates.length}
        </p>
                ...
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between">
                      <CardTitle className="text-sm font-semibold font-mono" dir="ltr">
                        {template.name}
                      </CardTitle>
                    ...
                    <div className="flex gap-1.5 mt-1 flex-wrap">
                      ...
                      <Badge variant="outline" className="text-[10px]">
                        {languageMap[template.language] || template.language}
                      </Badge>
                      {template.hasButtons && (
                        <Badge variant="outline" className="text-[10px]">
                          أزرار

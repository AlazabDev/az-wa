...
Deno.serve(async (req) => {
  ...
  const { data: num } = await admin.from("wa_numbers")
    .select("id,phone_number_id,tenant_id,meta").eq("id", wa_number_id).maybeSingle();
  ...
  if (!member || !["operator", "admin"].includes(String(member.role))) return json({ error: "Forbidden" }, 403);
  ...
  if (type === "text") {
    ...
    if (!template?.name || !template?.language?.code) return json({ error: "Template name and language are required" }, 400);
    ...
    if (!media?.link || !/^https:\/\//i.test(media.link)) return json({ error: "HTTPS media link required" }, 400);
  ...
  let { data: conv } = await admin.from("conversations")
    .select("id").eq("tenant_id", num.tenant_id).eq("wa_number_id", num.id).eq("contact_id", contact.id).maybeSingle();
  if (!conv) {
  ...
  const textValue = type === "text" ? String(text ?? "") : type === "reaction" ? (reaction?.emoji ?? "") : (media?.caption ?? null);
  const { error: insertError } = await admin.from("messages").insert({
    tenant_id: num.tenant_id, conversation_id: conv.id, direction: "outbound", status: "sent",
    type, text: textValue, provider_message_id: wamId, chat_id: toPhone,
    timestamp: new Date().toISOString(), sent_at: new Date().toISOString(),
    media_filename: media?.filename ?? null, raw_payload: waPayload,
  });

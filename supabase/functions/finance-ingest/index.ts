...
Deno.serve(async (req) => {
  ...
  if (!membership || !["operator", "admin"].includes(String(membership.role))) return json({ error: "Forbidden" }, 403);
  ...
  const { data: waNumber } = FINANCE_PHONE_ID
    ? await admin.from("wa_numbers").select("id").eq("tenant_id", tenantId).eq("phone_number_id", FINANCE_PHONE_ID).maybeSingle()
    : { data: null } as any;
  ...
  for (const file of (body.files ?? []).slice(0, 25)) {
    try {
      ...
      if (!mime.startsWith("image/")) throw new Error("Current finance OCR pipeline accepts image files only");
      ...
      if (dupe) { skipped.push(file.file_name ?? hash); continue; }

      const ext = (file.file_name?.split(".").pop() ?? mime.split("/")[1] ?? "jpg").toLowerCase().slice(0, 5);
      ...
      const { data: doc, error } = await admin.from("finance_documents").insert({
        tenant_id: tenantId, batch_id: batchId, wa_number_id: waNumber?.id ?? null,
        file_name: file.file_name ?? key, mime, size_bytes: bytes.length, sha256: hash,
        storage_provider: "minio", storage_bucket: MINIO_BUCKET, object_key: key, status: "pending",
      }).select("id").single();
  ...
  if (src && typeof src === "object") {
    ...
    const { data: objects, error: listError } = await admin.storage.from(bucket)
      .list(folder || undefined, { limit: pageSize, offset, sortBy: { column: "name", order: "asc" } });
    ...
    for (const obj of objects ?? []) {
      ...
      try {
        ...
        if (!mime.startsWith("image/")) { skipped.push(obj.name); continue; }
        ...
        const { data: dupe } = await admin.from("finance_documents")
          .select("id").eq("tenant_id", tenantId).eq("sha256", hash).maybeSingle();
        if (dupe) { skipped.push(obj.name); continue; }
        ...
        const { data: doc, error } = await admin.from("finance_documents").insert({
          tenant_id: tenantId, batch_id: batchId, wa_number_id: waNumber?.id ?? null,
          file_name: obj.name, mime, size_bytes: bytes.length, sha256: hash,
          storage_provider: "minio", storage_bucket: MINIO_BUCKET, object_key: key, status: "pending",
        }).select("id").single();
        if (error) throw new Error(error.message);
        created.push(doc.id);
  ...
  const { count } = await admin.from("finance_documents").select("id", { count: "exact", head: true }).eq("batch_id", batchId);
  await admin.from("finance_batches").update({ total_documents: count ?? 0, status: "processing" }).eq("id", batchId).eq("tenant_id", tenantId);

  return json({ success: true, batch_id: batchId, created: created.length, skipped: skipped.length, failed, next_offset: nextOffset });
});

...
async function resolveWaToken(waNumberId?: string | null) {
  if (waNumberId) {
    const { data } = await admin.from("wa_numbers").select("meta").eq("id", waNumberId).maybeSingle();
...
async function downloadWaMedia(mediaId: string, waNumberId?: string | null): Promise<{ bytes: Uint8Array; mime: string }> {
  const token = await resolveWaToken(waNumberId);
  if (!token) throw new Error("WhatsApp finance token not configured");
  const metaRes = await fetch(`https://graph.facebook.com/${WA_VERSION}/${mediaId}`, { headers: { Authorization: `Bearer ${token}` } });
...
async function rollupBatch(tenantId: string, batchId: string) {
  const [{ count: total }, { count: done }, { count: failedCount }] = await Promise.all([
    admin.from("finance_documents").select("id", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("batch_id", batchId),
    admin.from("finance_documents").select("id", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("batch_id", batchId).eq("status", "done"),
    admin.from("finance_documents").select("id", { count: "exact", head: true }).eq("tenant_id", tenantId).eq("batch_id", batchId).eq("status", "failed"),
  ]);
  const { data: sums } = await admin.from("finance_documents")
    .select("total_amount,currency").eq("tenant_id", tenantId).eq("batch_id", batchId).eq("status", "done");
  ...
  await admin.from("finance_batch_currency_totals").delete().eq("tenant_id", tenantId).eq("batch_id", batchId);
  if (grouped.size) {
    await admin.from("finance_batch_currency_totals").insert([...grouped.entries()].map(([currency, value]) => ({
      batch_id: batchId, tenant_id: tenantId, currency, total_amount: value.total, document_count: value.count,
    })));
  }
  ...
  await admin.from("finance_batches").update({
    ...
    status: (done ?? 0) + (failedCount ?? 0) >= (total ?? 0) && (total ?? 0) > 0 ? "completed" : "processing",
...
Deno.serve(async (req) => {
  ...
  if (batchId) {
    const { data: batch } = await admin.from("finance_batches").select("id")
      .eq("tenant_id", tenantId).eq("id", batchId).maybeSingle();
  ...
  if (resume && state?.is_paused) await admin.from("finance_worker_state").update({ is_paused: false, paused_reason: null }).eq("id", WORKER_ID);
  ...
  try {
    ...
    for (const doc of docs ?? []) {
      try {
        ...
        if (objectKey) {
          ...
          bytes = dl.bytes;
          mime = dl.mime;
          if (!mime.toLowerCase().startsWith("image/")) throw new Error(`Unsupported finance media type: ${mime}`);
          ...
          await admin.from("finance_documents").update({
            object_key: objectKey, storage_bucket: MINIO_BUCKET, mime, size_bytes: bytes.length, sha256: hash,
          }).eq("tenant_id", tenantId).eq("id", doc.id);
        } else {
        ...
        await admin.from("finance_documents").update({
          status: "done", ocr_text: vision.ocrText,
          vision_result: { caption: vision.caption, tags: vision.tags }, agent_result: extraction as any,
          doc_type: extraction.doc_type, vendor: extraction.vendor, invoice_number: extraction.invoice_number,
          invoice_date: extraction.invoice_date, currency: extraction.currency,
          total_amount: extraction.total_amount, tax_amount: extraction.tax_amount,
          confidence: extraction.confidence, error_message: null, processed_at: new Date().toISOString(),
        }).eq("tenant_id", tenantId).eq("id", doc.id);

        results.push({ id: doc.id, status: "done", provider: extraction.provider });
        if (probeOnly) await admin.from("finance_worker_state").update({ is_paused: false, paused_reason: null }).eq("id", WORKER_ID);
      } catch (e) {
        ...
        await admin.from("finance_documents").update({
          status: doc.attempts >= 3 ? "failed" : "pending", error_message: msg.slice(0, 500),
        }).eq("tenant_id", tenantId).eq("id", doc.id);
    ...
    if (paused || rateLimited >= 3) {
      await admin.from("finance_worker_state").update({
        is_paused: true, paused_reason: paused ?? "Repeated rate limiting; retry later",
      }).eq("id", WORKER_ID);
    }

    const remainingQuery = admin.from("finance_documents").select("id", { count: "exact", head: true })
      .eq("tenant_id", tenantId).eq("status", "pending");
    if (batchId) remainingQuery.eq("batch_id", batchId);
    const { count: remaining } = await remainingQuery;

    await admin.from("finance_worker_state").update({
      processed_total: (state?.processed_total ?? 0) + results.filter((r) => r.status === "done").length,
      last_error: null,
    }).eq("id", WORKER_ID);

    return json({ success: true, tenant_id: tenantId, batch_id: batchId, processed: results.length,
      remaining: remaining ?? 0, paused: Boolean(paused) || rateLimited >= 3, paused_reason: paused, results });
  } catch (e) {
